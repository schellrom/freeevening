...
public class MainGameSystem : MonoBehaviour
{
    ...
    {
        GAME_ENTRY,         // 최초 블록이 초기화
        GAME_WORK,          // 게임 클리어
        GAME_WORKING,       // 던전 이동
        GAME_READY,         // 적이 나를 공격하는건가?
        GAME_PLAYING,       // 블럭을 옮길 수 있는 상태
        GAME_SELECT_BLOCK,  // 블록 선택
        GAME_SWAP_BLOCK,    // 블록 스왑
    }
    ...
    {
        return (GameObject)Instantiate(Resources.Load("Prefab/particle/BlockDestroyParticle"));
    }

    public GameObject MakePlayerAttackParticle()
    {
        return (GameObject)Instantiate(Resources.Load("Prefab/particle/PlayerAttackParticle"));
    }
...
//              _stageLabel.gameObject.GetComponent<Animator>().Play("StageLabel");
    ...
    private GameState _tempGameState = GameState.GAME_ENTRY;
    public void Update()
    {
        updateShakingScreen();
        updateWorking();
        ...
        if (_tempGameState != _gameState)
        {
            _tempGameState = _gameState;
            Debug.Log ("_tempGameState : " + _tempGameState);
        }
    }
}